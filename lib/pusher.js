...
Pusher.prototype.trigger = function(channels, event, data, socketId, callback) {
  ...
  if (event.length > 200) {
    console.log("Too long event name: '" + event + "'");
    return
  }
  if (channels.length > 10) {
    // console.log("message has too many channels! Cutting them down to 10!");
    // console.log("Can't trigger a message to more than 10 channels");
    channels = channels.slice(0, 9);
  }
  for (var i = 0; i < channels.length; i++) {
    if (channels[i].length > 200) {
      // throw new Error("Too long channel name: '" + channels[i] + "'");
      console.log("Too long channel name: '" + channels[i] + "'");
      return
    }
    if (!channels[i].match(/^[a-zA-Z0-9_\-]+$/)) {
      // console.log("Invalid channel name: '" + channels[i] + "'");
      channels[i] = channels[i].replace(/[^\w-]|_/g, '-');
